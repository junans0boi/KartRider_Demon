...
public class New_Kart_Controller : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if((Input.GetKey(KeyCode.LeftShift) || Input.GetKey(KeyCode.RightShift)) && speedmetre >= 30 && (Input.GetAxisRaw("Horizontal")!=0)){
            rBody.angularDamping = 0;
            if(Input.GetAxisRaw("Horizontal") == 0)rBody.angularDamping = 5;
            isdrift = true;
        }else if(ratio >= 0.4f && speedmetre >= 30){
            rBody.angularDamping = 5;
            isdrift = true;
        }else{
            rBody.angularDamping = 0;
            isdrift = false;
        ...
        //spdcheck
        speedfloat = rBody.linearVelocity.magnitude;
        speedmetre = Mathf.RoundToInt(rBody.linearVelocity.magnitude * 6.8f);
        spd(speedmetre);
    ...
    private void FixedUpdate() {
        ...
        //wheelVelocityLS
        wheelVelocityLS = transform.InverseTransformDirection(rBody.linearVelocity); 
        ...
        if(isdrift){
            ...
            if(wheelVelocityLS.z < 0){
                ...
                rBody.linearVelocity = transform.TransformDirection(wheelVelocityLS);
